...
/*
void set_debug_reg(unsigned long va) {
	//write_debugreg(0, va);
	//write_debugreg(6, DEBUG_DR6);
	//write_debugreg(7, DEBUG_DR7);
	unsigned long dr0, dr6, dr7;
	set_debugreg(current, 0, va);
	set_debugreg(current, 6, DEBUG_DR6);
	set_debugreg(current, 7, DEBUG_DR7);
	dr0 = read_debugreg(0);
	dr6 = read_debugreg(6);
	dr7 = read_debugreg(7);
	printk("[joe]va:%lx dr0:%lx\n", va, dr0);
	printk("[joe]dr6:%lx\n", dr6);
	printk("[joe]dr7:%lx\n", dr7);
}
*/
                          ...
                          mfn_t sl4mfn)
{
    ...
    old_sl4e = *sl4e;
#ifdef AET_PF
	if ((sl4e->l4 & SH_L1E_AET_MAGIC) == SH_L1E_AET_MAGIC) {
    	_sp = sh_map_domain_page((sl4mfn));                  
    	...
    	{                                                                   
			...
			{
				if (sl4p->l4 == sl4e->l4) {
					sl4p->l4 &= (~SH_L1E_AET_MAGIC);
					printk("[joe] reverse in shadow_set_l4e sl4p:%p %lx\n", sl4p, sl4p->l4);
				}
			}
        }                                                               
...
*/		printk("[joe] shadow_set_l4e sl4e:%p %lx new_sl4e:%lx\n", sl4e, sl4e->l4, new_sl4e.l4);
	}
#endif
...
unsigned long long l4_aet_magic_count = 0;

static inline void reverse_l4_aet_magic(walk_t *gw, mfn_t sl4mfn) {
	shadow_l4e_t *sl4p;
	sl4p = sh_map_domain_page(sl4mfn);
	sl4p += shadow_l4_table_offset(gw->va);
	printk("[joe] reverse:l4:%p %lx\n", sl4p, sl4p->l4);
	sl4p->l4 &= (~SH_L1E_AET_MAGIC);
	sh_unmap_domain_page(sl4p);
}
...
static inline int sh_l2e_is_aet_magic(shadow_l2e_t sl2e) {
	return ((sl2e.l2 & SH_L1E_AET_MAGIC) == SH_L1E_AET_MAGIC);
}

static inline int sh_l3e_is_aet_magic(shadow_l3e_t sl3e) {
	return ((sl3e.l3 & SH_L1E_AET_MAGIC) == SH_L1E_AET_MAGIC);
}
...
//		}
	...
	*/
	...
	if ( ((sp->count_info & PGC_count_mask) != 0)
		...
		printk("sl1mfn:%lx is not use or type is not l1\n", sl1mfn);
		return;
	}
...
static inline void reverse_l1_aet_magic(mfn_t sl1mfn, unsigned long va/*, unsigned long error_code*/) {
	//shadow_l1e_t *sl1p;
	shadow_l1e_t *sl1e, *sl1e_start;
	sl1e_start = sh_map_domain_page(sl1mfn);
	sl1e = sl1e_start + l1_table_offset(va);
	//printk("[joe] reserve aet magic sl1e:%p %lx\n", sl1e, sl1e->l1);
	if (sh_l1e_is_aet_magic(*sl1e)) {
		add_reversed_aet_magic_count(va, sl1e->l1);
		//sl1e->l1 &= (~SH_L1E_AET_MAGIC);
		sl1e->l1 = 0;
...
/*
	...
	if (sh_l4e_is_aet_magic(*sl4e)) {
		...
		//if (count < 1000) {
			sl4e_p = sh_map_domain_page(sl4mfn);
			sl4e_p += shadow_l4_table_offset(gw->va);
			SHADOW_FOREACH_L4E(sl4mfn, sl4p, 0, 0, v->domain, {
				int flags = shadow_l4e_get_flags(*sl4p);
	//			printk("[joe] sl4p:%p %lx sl4e_p:%p %lx offset:%lx va:%lx\n", sl4p, sl4p->l4, sl4e_p, sl4e_p->l4, shadow_l4_table_offset(gw->va), gw->va);
				if ((flags & _PAGE_PRESENT) && (flags & _PAGE_USER)) {
		//			printk("[joe] get all l4, count:%llu sl4e:%p %lx sl4p:%p %lx\n", count, sl4e, sl4e->l4, sl4p, sl4p->l4);
					if (sl4e->l4 != sl4p->l4) {
						printk("[joe] should set sl4e:%p %lx sl4e_p:%p %lx sl4p:%p %lx count:%llx va:%lx\n", sl4e, sl4e->l4, sl4e_p, sl4e_p->l4, sl4p, sl4p->l4, l4_aet_magic_count, gw->va);
						sl4p->l4 |= SH_L1E_AET_MAGIC;
						l4_aet_magic_count++;
						done = 1;
					}
		...
		//}
		if (done == 1) {
			printk("[joe] l4 after set aet magic:sl4e:%p %lx\n", sl4e, sl4e->l4);
		}
	}
*/
#endif
...
/*
		if (done == 1) {
			printk("[joe] before shadow set l4e sl4e:%p %lx new_sl4e:%lx\n", sl4e, sl4e->l4, new_sl4e.l4);
		}
*/
#endif
...
/*
		if (done == 1) {
			printk("[joe] after shadow set l4e sl4e:%p %lx new_sl4e:%lx\n", sl4e, sl4e->l4, new_sl4e.l4);
		}
*/
#endif
                                                ...
                                                int *resync)
{
	...
	if (sh_l3e_is_aet_magic(*sl3e)) {
		printk("[joe] weird l3e is aet magic sl3e:%p %lx\n", sl3e, sl3e->l3);
	}
...
#if GUEST_PAGING_LEVELS == 4
	if (sh_l2e_is_aet_magic(*sl2e)) {
		printk("[joe] weird l2e is aet magic sl2e:%p %lx\n", sl2e, sl2e->l2);
	}
#endif
...
//			printk("reserved bit error va:%lx sl4mfn:%lx\n", va, sl4mfn);
		...
		if (sl4e_temp->l4 & SH_L1E_AET_MAGIC) {
			...
			printk("[joe] big big surprise!!! reverse sl4e_temp:%p %lx va:%lx error_code:%x\n", sl4e_temp, sl4e_temp->l4, va, regs->error_code);
		}
	...
	//			printk("[joe] big big surprise!!! reverse sl4e:%p %lx\n", sl4e, sl4e->l4);
		...
		if (count != 0)
			printk("[joe] big big surprise!!! reverse other sl4e count:%d\n", count);
		sh_unmap_domain_page(sl4e_start);
	}

	if (is_l1_track() || is_page_fault_count()) {
		add_page_fault_count();
	}
/*
	if ( (regs->error_code & PFEC_reserved_bit) && (regs->error_code & PFEC_user_mode )) {
		printk("[joe] user mode error va:%lx error_code:%x\n", va, regs->error_code);
	}
	*/
#endif
#endif
...
*/
 ...
 rewalk:
	...
	if (sh_l1e_is_aet_magic(*ptr_sl1e)) {
		printk("[WARNING1!!!!!!] is aet magic va:%lx ptr_sl1e:%p %lx\n", va, ptr_sl1e, ptr_sl1e->l1);
	}
#endif
#endif
	...
	/*
	if (sh_l1e_is_aet_magic(*ptr_sl1e)) {
		printk("[WARNING2!!!!!!] is aet magic va:%lx ptr_sl1e:%p %lx\n", va, ptr_sl1e, ptr_sl1e->l1);
	}*/
#endif
#endif
 ...
 * to do so. */
{
...
#if SHADOW_PAGING_LEVELS == 4
    {
		...
		//if (is_aet_track_open()) {
			if (sh_l4e_is_aet_magic(sh_linear_l4_table(v)[shadow_l4_linear_offset(va)])) {
				mfn_t sl4mfn;
				shadow_l4e_t *sl4e_start, *sl4e_temp;
				printk("[joe] sl4e is aet magic in invlpg!!!!!!!! sl4e:%p\n", sh_linear_l4_table(v) + shadow_l4_linear_offset(va));
				sl4mfn = pagetable_get_mfn(v->arch.shadow_table[0]);
				sl4e_start = sh_map_domain_page(sl4mfn);
				sl4e_temp = sl4e_start + l4_table_offset(va);
				if (sl4e_temp->l4 & SH_L1E_AET_MAGIC) {
					sl4e_temp->l4 &= (~SH_L1E_AET_MAGIC);
					printk("[joe] big big surprise in invlpg!!! reverse sl4e_temp:%p %lx va:%lx\n", sl4e_temp, sl4e_temp->l4, va);
				}
